...
def get_active_monitors(monitor_ids: List[int] = None, in_dataframe: bool = False) -> QuerySet[Monitor, Monitor] | DataFrame:
    if monitor_ids:
        queryset = Monitor.objects.select_related('user').filter(id__in=monitor_ids, is_active=True)
    else:
